...
class ConditionJudge{
	...
	public:
		ConditionJudge(string _conditionStr,map<string,VariableInfo> _varMap){
			...
			size_t equalPos = _conditionStr.find('=');
			size_t stringPos = _conditionStr.find('"');
			map<string,VariableInfo>::iterator conditionMI;
			if(equalPos != string::npos){
				//Store left and right conditions of equal sign
				string leftVar = _conditionStr.substr(0,equalPos-0);
				string rightVar = _conditionStr.substr(equalPos+1);
				string leftVal;
				string rightVal;
				//Find the leftVar value
				if(leftVar[0] - '0'<49 && stringPos == string::npos){
					//int or float
					size_t floatPos = _conditionStr.find('.');
					...
					//string
					leftVal = leftVar;
				} else if(leftVar[0] - '0'>=49 && stringPos == string::npos){
					//variable
					conditionMI = varMap.find(leftVar);
					leftVal = (*conditionMI).second.value;
				}
				//Find the rightVar value
				if(rightVar[0] - '0'<49 && stringPos == string::npos){
					//int or float
					size_t floatPos = _conditionStr.find('.');
					if(floatPos == string::npos){
						//int
					...
					//string
					rightVal = rightVar;
				} else if(rightVar[0] - '0'>=49 && stringPos == string::npos){
					//variable
					conditionMI = varMap.find(rightVar);
					rightVal = (*conditionMI).second.value;
				}
				//Judge the TorF value
				if(leftVal == rightVal){
					TorF = 1;
				} else{
					TorF = 0;
			...
			while(!_stringQueue.empty()){
				...
				switch(commandType){
					...
					case 2:{
						...
						while(toContinue){
							...
							if(conditionStr == "elseif"){
								if(ifState == 1){
									...
									cout<<"if "<<conditionStatement<<", then:"<<endl;
									queue<string> tempIfStrQueue = tempIfStr;
									while(!tempIfStrQueue.empty()){
										cout<<tempIfStrQueue.front()<<endl;
										tempIfStrQueue.pop();
									}
									ifElseQueue.push(make_pair("if",make_pair(grandCondition,ifWordSepSys)));
									//Clear the string queue
									while(!tempIfStr.empty()){
										tempIfStr.pop();
									}
									ifState = 2;
								} else if(ifState == 2){
									...
									//Construct the ConditionJudge object for determining whether to use it
									ConditionJudge grandCondition(conditionStatement,varMap);
									cout<<"elseif "<<conditionStatement<<", then:"<<endl;
									queue<string> tempIfStrQueue = tempIfStr;
									while(!tempIfStrQueue.empty()){
										cout<<tempIfStrQueue.front()<<endl;
										tempIfStrQueue.pop();
								...
								//Construct the ConditionJudge object for determining whether to use it
								ConditionJudge grandCondition(conditionStatement,varMap);
								cout<<"else "<<conditionStatement<<", then:"<<endl;
								queue<string> tempIfStrQueue = tempIfStr;
								while(!tempIfStrQueue.empty()){
									cout<<tempIfStrQueue.front()<<endl;
									tempIfStrQueue.pop();
						...
						cout<<"with orders:"<<endl;
						queue<string> tempOrderQueue = rightConditionToPerform.getOrderQueue();
						while(!tempOrderQueue.empty()){
